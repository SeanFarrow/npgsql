...
{
    /// <summary>
    /// Provides a basic, low level API for a replication operation.
    /// </summary>
    /// <remarks>See https://www.postgresql.org/docs/current/static/protocol-replication.html for more details.</remarks>
    public class NpgsqlRawReplicationStream : Stream, ICancelable
    {
        ...
        [PublicAPI]
        public NpgsqlReplicationStreamFetchStatus FetchNext() => FetchNext(false, false).Result;

        /// <inheritdoc cref="FetchNext()"/> 
        [PublicAPI]
        public async ValueTask<NpgsqlReplicationStreamFetchStatus> FetchNextAsync() => await FetchNext(false, true);
                    ...
                    switch (msg.Code)
                    {
                        ...
                        case BackendMessageCode.CompletedResponse:
                            ...
                            if (_copyInMode || _copyOutMode)
                            {
                                ...
                                throw new InvalidOperationException("Unexpected message received from server. If this connection already ran a replication stream before, it should be discarded. Replication connections can only run a replication stream once.");
                            }
                            return NpgsqlReplicationStreamFetchStatus.Closed;
                        default:
                            throw _connector.UnexpectedMessageReceived(msg.Code);
        ...
        public override async Task<int> ReadAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken) => 
            await Read(buffer, offset, count, true);

        /// <inheritdoc />
        public override int Read(byte[] buffer, int offset, int count) => 
            Read(buffer, offset, count, false).Result;
        ...
        void ICancelable.Cancel() => Close(false);
